# Spec Delta: tools (add-od-save-project-file)

Adds a new MCP tool `od_save_project_file` that wraps the OD daemon's `POST /api/projects/:id/files` endpoint, enabling agents to persist a generated file inside an existing project so it appears in `od_get_project.files[]` and renders in the daemon's project UI.
